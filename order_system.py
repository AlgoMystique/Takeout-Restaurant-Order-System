def place_order(menu):
    ...
    while place_order:
        ...
        for category, options in menu.items():
            ...
            # meal and the price
            for i, (meal, price) in enumerate(options.items(), 1):
        ...
        # Check if the customer types 'n' or 'N'
...
def update_order(order, menu_selection, menu_items):
    ...
    try:
        ...
        # If conversion fails, print an error message with the invalid input
    ...
    if menu_selection in menu_items:
        ...
        # Prompt the customer that prints the name of the menu item to the user 
        # and asks the quantity they would like to order.
        quantity = input(f"How many {item_name} would you like to order? ")

        try:
            # Convert the quantity to an integer, default to 1 if invalid
            quantity = int(quantity)
        except ValueError:
            quantity = 1  # Default to 1 if the input is not a number

        # Add the dictionary with the item name, price, and quantity to the order list
...
if __name__ == "__main__":
    ...
    # Print out each item in the order
    print_itemized_receipt(receipt)

    # Print the footer with the total price
